import scanpy as sc  # 处理单细胞RNA-seq数据
import numpy as np  # 数值操作
import torch
import torch.optim as optim  # PyTorch 构建和训练神经网络
from torch.nn import Parameter  # 创建模型参数
from torch.utils.data import DataLoader  # 创建数据加载器
from tqdm import tqdm  # 在训练过程中创建进度条


# 导入了一些自定义模块(.model、.util)
...
# run_scace 用于在scRNA-seq数据上训练scAce模型
def run_scace(adata: sc.AnnData,
    ...
    if return_all:
        pred_all, emb_all = [], []  # 如果设置了return_all为True，代码将创建两个空列表pred_all和emb_all，用于存储和打印中间结果

    device = 'cuda' if torch.cuda.is_available() else 'cpu'  # 选择计算设备，是否有可用的CUDA兼容GPU，如果有的话，选择使用GPU，否则使用CPU
    ...
    scace_model = scAce(input_dim=input_dim, device=device, z_dim=z_dim, encode_layers=encode_layers,
                        decode_layers=decode_layers, activation=activation)
    ...
    for epoch in tqdm(range(n_epochs), unit='epoch', desc='Training:'):
        ...
        if epoch > 0 and (delta_label < tol or epoch % 20 == 0):
            ...
            Final_Centroid_merge, Label_merge, n_clusters_t, pred_t = centroid_merge(scace_emb, Centroid, y_pred, d_bar,
                                                                                     intra_dis, d_ave)
            m_numbers += 1
